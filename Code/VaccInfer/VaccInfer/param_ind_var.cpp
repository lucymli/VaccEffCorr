...
//  param_ind_var.cpp
//  VaccInfer
//  Fit a single model to all serotype-specific antibody levels
//
//  Created by Lucy Li on 4/11/17.
...
void calc_expm(bool vacc, int ind, Data data, arma::mat& original_matrix, arma::mat& transitions1, double multiplier, int n_tot, int n_vtypes, std::vector<double> p0, std::vector <double> thetaSI, bool use_mean_ab) {
    ...
    for (int i=0; i<n_tot; i++) {
        ...
        if (vacc & (i<n_vtypes) & (multiplier < STATIONARY_TIME)) {
            ...
            matrix_to_change(0, i+1) *= p0[0]-susceptibility*thetaSI[0];//*full_immunity*ind_frailty_SI[(i-1)*n_vacc+ind]
        ...
        for (int col_i=1; col_i<=n_tot; col_i++) {
            if (row_i!=col_i) {
                ...
                if (vacc & (col_i<=n_vtypes) & (multiplier < STATIONARY_TIME)) {
                    ...
                    //matrix_to_change(row_i, col_i) *= p0[0]-susceptibility*thetaSI[0];//*full_immunity*ind_frailty_SI[(col_i-1)*n_vacc+ind]
                    matrix_to_change(row_i, col_i) *= p0[0]*std::exp(-susceptibility*thetaSI[0]);
                }
...
void Param::print_to_file (std::string filename, int iter, arma::mat &output_mat) {
    ...
    for (int i=0; i<n_params; i++) {
        if (i > n_tot*2 & i < n_tot*2+n_vtypes) continue;
        if (i > n_tot*2+n_vtypes & i < n_tot*2+n_vtypes*2) continue;
        output_file << "\t" << (*this)[i];
