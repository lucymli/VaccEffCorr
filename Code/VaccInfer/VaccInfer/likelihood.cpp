...
double predict_lambda (Param &parameters, Data data) {
    double a, b;
    for (int type_i=0; type_i<parameters.n_vtypes; type_i++) {
        ...
        for (int ind_i=0; ind_i<data.n_ind; ind_i++) {
            parameters.set_inferred_risk(a*exp(b*data.get_predictor(ind_i, type_i)), ind_i, type_i);
        }
    }
}
...
void fill_rates (Param parameters, arma::mat & mat) {
    double tot_rate = 0.0;
    for (int i=1; i<=parameters.n_tot; i++) {
        mat(i, 0) = parameters.get_param(1, i+1); // mu
        mat(0, i) = parameters.get_param(0, i+1); // lambda
        tot_rate += mat(0, i);
    }
    double competition;
...
double calc_llik (Param &parameters, Data &data) {
    int nthread = 4;//omp_get_num_threads();
    predict_lambda(parameters, data);
...
//            arma::mat ind_mat = arma::expmat(fill_rates(base, data, parameters, ind_i)*100.0);
//            llik_vec[tn] += ind_mat[0, data.get_carriage(ind_i, 0)];
//            std::cout << "A " << ind_i << " " << llik_vec[tn] << std::endl;
//        }
//    }
    double time_diff;
    int previous, now;
